...
export function HomeClient({ initialShops }: HomeClientProps) {
  const [address, setAddress] = useState("Detectando...");
  ...
  useEffect(() => {
    // Auto-detect location on mount with reverse geocoding
          ...
          if (apiKey) {
            try {
              ...
              if (data.results?.[0]) {
                ...
                const city = components?.find((c: any) => c.types.includes("administrative_area_level_2"))?.short_name || components?.find((c: any) => c.types.includes("locality"))?.short_name;
              ...
              setAddress("Sua Localização");
            }
          }
        },
        (error) => {
          console.warn("Location access denied:", error);
          setAddress("Selecionar Local");
        },
  ...
  return (
    <div className="flex flex-col min-h-screen pb-24 bg-[#0D0D1F] text-foreground font-sans overflow-x-hidden">
          ...
          <div className="mt-4 px-2 animate-in slide-in-from-top-4 duration-500">
            <div className="glass p-4 rounded-[2rem]">
              <LocationSearch
                ...
                initialValue={address === "Detectando..." ? "" : address}
                autoDetect={true}
              />
            </div>
          </div>
      ...
      <main className="flex-1 px-6 pt-2">
        ...
        {/* Feature Hero (only shown when no research/filter active) */}
          ...
          {loading ? (
            ...
            shops.map((shop) => (
              <ServiceCard
                ...
                title={shop.name}
                rating={shop.rating}
                reviews={((shop.id?.charCodeAt(0) || 0) % 20) + 5}
                price="$$$"
